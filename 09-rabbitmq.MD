...
Create `/etc/yum.repos.d/rabbitmq.repo`.
...
RabbitMQ comes with a default username and password as `guest/guest`. But this user cannot be used to connect. Hence, we need to create one user for the application.
